...
vec3 materialNormal(){
  ...
  normal=v_tangentToWorld*(normal*2.-1.);
  ...
  #endif
  normal=v_tangentToWorld*(SAMPLER_FNC(MATERIAL_BUMPMAP_NORMALMAP,uv).xyz*2.-1.);
  #endif
  #endif
...
#define TOMETALLIC_MIN_REFLECTANCE .04
#endif
...
float toMetallic(vec3 diffuse,vec3 specular,float maxSpecular){
  float perceivedDiffuse=sqrt(.299*diffuse.r*diffuse.r+.587*diffuse.g*diffuse.g+.114*diffuse.b*diffuse.b);
  float perceivedSpecular=sqrt(.299*specular.r*specular.r+.587*specular.g*specular.g+.114*specular.b*specular.b);
  if(perceivedSpecular<TOMETALLIC_MIN_REFLECTANCE){
    return 0.;
  }
  float a=TOMETALLIC_MIN_REFLECTANCE;
  float b=perceivedDiffuse*(1.-maxSpecular)/(1.-TOMETALLIC_MIN_REFLECTANCE)+perceivedSpecular-2.*TOMETALLIC_MIN_REFLECTANCE;
...
float toShininess(float roughness,float metallic){
  float s=.95-roughness*.5;
  s*=s;
  s*=s;
  return s*(80.+160.*(1.-metallic));
}
...
vec3 fresnel(const vec3 f0,float NoV){
  ...
  return schlick(f0,1.,NoV);
  #else
  float f90=saturate(dot(f0,vec3(50.*.33)));
...
float luminance(in vec3 linear){return dot(linear,vec3(.2126,.7152,.0722));}
...
vec3 tonemapDebug(const vec3 x){
  ...
  vec3 debugColors[17];
  debugColors[0]=vec3(0.,0.,0.);// black
  debugColors[1]=vec3(0.,0.,.1647);// darkest blue
  debugColors[2]=vec3(0.,0.,.3647);// darker blue
  debugColors[3]=vec3(0.,0.,.6647);// dark blue
  debugColors[4]=vec3(0.,0.,.9647);// blue
  debugColors[5]=vec3(0.,.9255,.9255);// cyan
  debugColors[6]=vec3(0.,.5647,0.);// dark green
  debugColors[7]=vec3(0.,.7843,0.);// green
  debugColors[8]=vec3(1.,1.,0.);// yellow
  debugColors[9]=vec3(.90588,.75294,0.);// yellow-orange
  debugColors[10]=vec3(1.,.5647,0.);// orange
  debugColors[11]=vec3(1.,0.,0.);// bright red
  debugColors[12]=vec3(.8392,0.,0.);// red
  debugColors[13]=vec3(1.,0.,1.);// magenta
  debugColors[14]=vec3(.6,.3333,.7882);// purple
  debugColors[15]=vec3(1.,1.,1.);// white
  debugColors[16]=vec3(1.,1.,1.);// white
  
  // The 5th color in the array (cyan) represents middle gray (18%)
  // Every stop above or below middle gray causes a color shift
  float v=log2(luminance(x)/.18);
  v=clamp(v+5.,0.,15.);
  int index=int(v);
...
vec3 tonemapReinhard(const vec3 x){return x/(1.+luminance(x));}
...
vec3 tonemapUncharted(const vec3 x){
  ...
  vec3 whiteScale=1./uncharted2Tonemap(vec3(W));
...
vec3 tonemapUnreal(const vec3 x){return x/(x+.155)*1.019;}
...
vec3 fakeCube(vec3 _normal,float _shininnes){
  ...
  return vec3(powFast(max(max(rAbs.x,rAbs.y),rAbs.z)+.005,_shininnes));
}
...
vec3 sphericalHarmonics(const vec3 n){
  ...
  return SPHERICALHARMONICS_TONEMAP(max(
      .282095*SCENE_SH_ARRAY[0]
      #if SPHERICALHARMONICS_BANDS>=2
      +-.488603*SCENE_SH_ARRAY[1]*(n.y)
      +.488603*SCENE_SH_ARRAY[2]*(n.z)
      +-.488603*SCENE_SH_ARRAY[3]*(n.x)
      #endif
      #if SPHERICALHARMONICS_BANDS>=3
      +1.092548*SCENE_SH_ARRAY[4]*(n.y*n.x)
      +-1.092548*SCENE_SH_ARRAY[5]*(n.y*n.z)
      +.315392*SCENE_SH_ARRAY[6]*(3.*n.z*n.z-1.)
      +-1.092548*SCENE_SH_ARRAY[7]*(n.z*n.x)
      +.546274*SCENE_SH_ARRAY[8]*(n.x*n.x-n.y*n.y)
      #endif
    ,0.));
    #else
    return vec3(1.);
  ...
  vec3 envMap(vec3 _normal,float _roughness){
    return envMap(_normal,_roughness,1.);
  }
  #endif
  ...
  vec3 fresnelReflection(vec3 R,vec3 f0,float NoV){
    ...
    reflectColor=ENVMAP_FNC(R,.001,.001);
  ...
  float specularPhong(vec3 L,vec3 N,vec3 V,float shininess){
    ...
    return SPECULAR_POW(max(0.,dot(R,-V)),shininess);
  }
  
  float specularPhongRoughness(vec3 L,vec3 N,vec3 V,float roughness){
    return specularPhong(L,N,V,toShininess(roughness,0.));
  }
  ...
  float specularBlinnPhong(vec3 L,vec3 N,vec3 V,float shininess){
    ...
    return SPECULAR_POW(max(0.,dot(N,H)),shininess);
  }
  
  float specularBlinnPhongRoughnes(vec3 L,vec3 N,vec3 V,float roughness){
    return specularBlinnPhong(L,N,V,toShininess(roughness,0.));
  }
  ...
  #endif
  #ifndef ONE_OVER_PI
  #define ONE_OVER_PI .31830988618
  #endif
  #ifndef SQRT_HALF_PI
  ...
  #define GOLDEN_RATIO_CONJUGATE .61803398875
  #endif
  ...
  float GGX(float NoH,float roughness){
    float oneMinusNoHSquared=1.-NoH*NoH;
  ...
  float specularCookTorrance(vec3 _L,vec3 _N,vec3 _V,float _NoV,float _NoL,float _roughness,float _fresnel){
    float NoV=max(_NoV,0.);
    float NoL=max(_NoL,0.);
    
    // Half angle vector
    vec3 H=normalize(_L+_V);
    
    // Geometric term
    float NoH=max(dot(_N,H),0.);
    float VoH=max(dot(_V,H),.000001);
    
    float x=2.*NoH/VoH;
    float G=min(1.,min(x*NoV,x*NoL));
    
    // Distribution term
    float D=SPECULAR_COOKTORRANCE_DIFFUSE_FNC(NoH,_roughness);
    
    // Fresnel term
    float F=SPECULAR_POW(1.-NoV,_fresnel);
    
    // Multiply terms and done
    return max(G*F*D/max(PI*NoV*NoL,.00001),0.);
  }
  ...
  float specularCookTorrance(vec3 L,vec3 N,vec3 V,float roughness){
    return specularCookTorrance(L,N,V,roughness,.04);
  }
  #endif
  ...
  float smithGGXCorrelated_Fast(float NoV,float NoL,float roughness){
    ...
    float v=.5/mix(2.*NoL*NoV,NoL+NoV,roughness);
    return saturateMediump(v);
  }
  #endif
  ...
  float specularGGX(vec3 L,vec3 N,vec3 V,float roughness){
    return specularGGX(L,N,V,roughness,.04);
  }
  ...
  float diffuseLambert(vec3 L,vec3 N){return max(0.,dot(N,L));}
  ...
  float diffuseBurley(float NoV,float NoL,float LoH,float linearRoughness){
    ...
    float f90=.5+2.*linearRoughness*LoH*LoH;
    float lightScatter=schlick(1.,f90,NoL);
    float viewScatter=schlick(1.,f90,NoV);
    return lightScatter*viewScatter;
  }
  
  float diffuseBurley(vec3 L,vec3 N,vec3 V,float NoV,float NoL,float roughness){
    float LoH=max(dot(L,normalize(L+V)),.001);
  ...
  float diffuseBurley(vec3 L,vec3 N,vec3 V,float roughness){
    ...
    float NoL=clamp(dot(N,L),.001,1.);
    float LoH=clamp(dot(L,H),.001,1.);
  ...
  float falloff(float _dist,float _lightRadius){
    float att=clamp(1.-_dist*_dist/(_lightRadius*_lightRadius),0.,1.);
    att*=att;
    return att;
  }
  #endif
  ...
  #define SURFACE_POSITION vec3(0.,0.,0.)
  #endif
  
  #ifndef LIGHT_POSITION
  #define LIGHT_POSITION vec3(0.,10.,-50.)
  #endif
  
  #ifndef LIGHT_COLOR
  #define LIGHT_COLOR vec3(.5,.5,.5)
  #endif
  ...
  void lightPoint(vec3 _diffuseColor,vec3 _specularColor,vec3 _N,vec3 _V,float _NoV,float _roughness,float _f0,float _shadow,inout vec3 _diffuse,inout vec3 _specular){
    ...
    _diffuse+=max(vec3(0.),_diffuseColor*lightContribution*dif);
    _specular+=max(vec3(0.),_specularColor*lightContribution*spec);
  }
  ...
  void lightPoint(vec3 _diffuseColor,vec3 _specularColor,vec3 _N,vec3 _V,float _NoV,float _roughness,float _f0,inout vec3 _diffuse,inout vec3 _specular){
    lightPoint(_diffuseColor,_specularColor,_N,_V,_NoV,_roughness,_f0,1.,_diffuse,_specular);
  }
  ...
  void lightDirectional(vec3 _diffuseColor,vec3 _specularColor,vec3 _N,vec3 _V,float _NoV,float _roughness,float _f0,float _shadow,inout vec3 _diffuse,inout vec3 _specular){
    ...
    _diffuse+=max(vec3(0.),LIGHT_INTENSITY*(_diffuseColor*LIGHT_COLOR*dif)*_shadow);
    _specular+=max(vec3(0.),LIGHT_INTENSITY*(_specularColor*LIGHT_COLOR*spec)*_shadow);
  }
  ...
  vec3 reflection(vec3 _V,vec3 _N,float _roughness){
    ...
    vec3 anisotropyDirection=MATERIAL_ANISOTROPY>=0.?anisotropicB:anisotropicT;
    vec3 anisotropicTangent=cross(anisotropyDirection,_V);
    vec3 anisotropicNormal=cross(anisotropicTangent,anisotropyDirection);
    float bendFactor=abs(MATERIAL_ANISOTROPY)*saturate(5.*_roughness);
  ...
  vec4 pbr(const Material _mat){
    // Calculate Color
    vec3 diffuseColor=_mat.albedo.rgb*(vec3(1.)-_mat.f0)*(1.-_mat.metallic);
    ...
    float specIntensity=(2.*_mat.metallic)*
    saturate(-1.1+NoV+_mat.metallic)*// Fresnel
    (_mat.metallic+(.95-_mat.roughness)*2.);// make smaller highlights brighter
    ...
    Fr+=tonemap(fresnelReflection(R,_mat.f0,NoV))*_mat.metallic*(1.-_mat.roughness)*.2;
    Fr*=specularAO;
  ...
  float sampleShadow(in sampler2D shadowMap,in vec3 _coord){
    return sampleShadow(shadowMap,vec4(_coord,1.));
  }
  ...
  float sampleShadowLerp(sampler2D depths,vec2 size,vec2 uv,float compare){
    ...
    vec2 f=fract(uv*size+.5);
    vec2 centroidUV=floor(uv*size+.5)/size;
    float lb=sampleShadow(depths,centroidUV+texelSize*vec2(0.,0.),compare);
    float lt=sampleShadow(depths,centroidUV+texelSize*vec2(0.,1.),compare);
    float rb=sampleShadow(depths,centroidUV+texelSize*vec2(1.,0.),compare);
    float rt=sampleShadow(depths,centroidUV+texelSize*vec2(1.,1.),compare);
  ...
  void materialNew(out Material _mat){
    ...
    _mat.f0=vec3(.04,.04,.04);// reflectance at 0 degree
    ...
    // #if defined(MATERIAL_HAS_CLEAR_COAT)
    _mat.clearCoat=0.;
    _mat.clearCoatRoughness=.01;
    #if defined(MATERIAL_HAS_CLEAR_COAT_NORMAL)
    _mat.clearCoatNormal=vec3(0.,0.,1.);
    #endif
    // #endif
    ...
    #if defined(MATERIAL_SUBSURFACE_COLOR)
    #if defined(SHADING_MODEL_SUBSURFACE)
    _mat.subsurfaceColor=vec3(1.,1.,1.);
    #else
    _mat.subsurfaceColor=vec3(0.,0.,0.);
    #endif
    #endif
  ...
  float checkBoard(vec2 uv,vec2 _scale){
    uv=floor(fract(uv*_scale)*2.);
    return min(1.,uv.x+uv.y)-(uv.x*uv.y);
  }
  ...
  void main(void){
    ...
    material.albedo.rgb=vec3(.5)+checkBoard(v_texcoord,vec2(8.))*.5;
    #endif

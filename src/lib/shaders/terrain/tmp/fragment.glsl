...
uniform sampler2D uSiteTexture;
...
{
  // Displace the UV
  vec2 displacedUv=vUv+cnoise(vec3(vUv*5.,uTime*uTimeFrequency*.4));
  
  // Perlin noise
  float strenght=cnoise(vec3(displacedUv*5.,uTime*uTimeFrequency*.7));
  
  // Outer glow
  float outerGlow=distance(vUv,vec2(.5))*4.-1.4;
  strenght+=outerGlow;
  
  // Apply cool step
  strenght+=step(-.2,strenght)*.8;
  ...
  vec3 siteTexture=texture2D(uSiteTexture,vUv).rgb;
  vec3 colorCenter=mix(uColorCenter,siteTexture,uActiveMix);
  vec3 colorPortal=mix(colorCenter,uColorBorder,strenght);
  
  vec3 mixColor=mix(bakedNeutral,colorPortal,uNeutralMix);
